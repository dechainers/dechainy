...
# queue to share tasks with Lucid
...
class Lucid(Process):
    ...
    def __init__(self):
        ...
        self.maxs = np.array([x[1] for x in feature_list.values()])
        self.rng = np.array([x[1] - x[0] for x in feature_list.values()])

    def run(self):
        global shared_queue, model_path
        ...
        while True:
            ...
            if not val:
                break
            self.async_prediction(*val)

    def normalize_and_padding_sample(self, sample: np.array, high: float = 1.0, low: float = 0.0) -> np.array:
    ...
    def async_prediction(self, packets, exec_number, checkpoint_0, checkpoint_1, checkpoint_2, total_pkts_passed_from_sessions, compute_ids=False):
        ids_list = []
        data = []
        checkpoint_3 = time.time()
        for k, v in groupby(sorted(packets, key=lambda x: x[0]), key=lambda x: x[0]):
            if compute_ids:
            ...
            data.append(self.normalize_and_padding_sample(np.array([x[1] for x in v])))
        data = np.array(data)
        data = np.expand_dims(data, axis=3)
        checkpoint_4 = time.time()
        prediction = self.model.predict(data, batch_size=2048) > 0.5
        ...
        print(f"Execution n°{exec_number}:"
            ...
            f"\n\tTIME sending-receiving data to process: {checkpoint_3 - checkpoint_2}"
...
def reaction_function_rest(probe: Plugin) -> Dict[str, any]:
    ...
    if packets:
        pool.apply_async(queue_async_put, (packets, exec_number, checkpoint_0, checkpoint_1, checkpoint_2, total_pkts_passed_from_sessions,))
        return True
    return False


def queue_async_put(packets, exec_number, checkpoint_0, checkpoint_1, checkpoint_2, total_pkts_passed_from_sessions):
    global shared_queue
    shared_queue.put(([((val.id.saddr, val.id.sport, val.id.daddr, val.id.dport, val.id.proto), [getattr(val, x) for x in feature_list]) for val in packets], exec_number, checkpoint_0, checkpoint_1, checkpoint_2, total_pkts_passed_from_sessions))
